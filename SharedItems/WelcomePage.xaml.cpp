...
#include "..\MisHorariosCpp_UWP\utils\Helpers.h"
#include <fstream>
#include <thread>
#include <pplawait.h>
#include <string.h>
...
static String^ CurrentVersionNumber = "v1.48";
static int CurrentVersionNumberInt = 148;
...
void WelcomePage::OnNavigatedTo(NavigationEventArgs^ e)
{
	...
	//APP_TITLE->Text = "Mis Horarios BK";
...
void WelcomePage::CheckUpdates(void)
{
	...
	try
	{
		...
		create_task(client->GetAsync(ref new Uri("https://---.COM/version/last.txt"))).then([=](HttpResponseMessage^ response)
		{
